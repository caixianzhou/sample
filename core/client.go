...
// Package core MidasBuy API Go SDK HTTPClient 基础库，你可以使用它来创建一个 Client，并向MidasBuy发送 HTTP 请求
//
// 初始化 Client 时，你需要指定以下参数：
//   - Credential 用于生成 HTTP Header 中的 Authorization 信息，MidasBuy API依赖该值来保证请求的真实性和数据的完整性
//   - Validator 用于对MidasBuy的应答进行校验，避免被恶意攻击
package core
...
// ClientOption MidasBuy API HTTPClient core.Client 初始化参数
...
// NewClient 初始化一个MidasBuyAPI HTTPClient
//
// 初始化的时候你可以传递多个配置信息
...
// NewClientWithDialSettings 使用 DialSettings 初始化一个MidasBuyAPI HTTPClient
...
func initClientWithSettings(_ context.Context, settings *DialSettings) *Client {
	client := &Client{
		...
		credential: &credentials.MidasBuyCredentials{Signer: settings.Signer},
...
func initSettings(opts []ClientOption) (*DialSettings, error) {
	...
	if o.Validator == nil {
		o.Validator = &validators.NullValidator{}
	}
...
// Post 向MidasBuy发送一个 HTTP Post 请求
...
// Patch 向MidasBuy发送一个 HTTP Patch 请求
...
// Delete 向MidasBuy发送一个 HTTP Delete 请求
...
func (client *Client) doRequest(
	...
	if authorization, err = client.credential.GenerateAuthorizationHeader(
		ctx, consts.AuthIDTypeAppID, method, request.URL.RequestURI(),
		signBody,
	); err != nil {
	...
	// indicate MidasBuy-Serial that client can verify
	if serial, err := client.validator.GetAcceptSerial(ctx); err == nil {
		request.Header.Set(consts.MidasBuySerial, serial)
	}
