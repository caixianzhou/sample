...
var (
	...
	MidasBuyPrivateKey  *rsa.PrivateKey
	MidasBuyCertificate *x509.Certificate
	signer              auth.Signer
...
func init() {
	...
	MidasBuyCertificate, err = utils.LoadCertificate(testMidasBuyCertificateStr)
	if err != nil {
		panic(fmt.Errorf("generate MidasBuy testing certificate err:%s", err.Error()))
	}
	MidasBuyPrivateKey, err = utils.LoadPrivateKey(testingKey(testMidasBuyPrivateKeyStr))
	if err != nil {
		panic(fmt.Errorf("generate MidasBuy testing key err:%s", err.Error()))
	}
}
...
func writeSignature(w http.ResponseWriter, body string) {
	w.Header().Set(consts.RequestID, "0")
	w.Header().Set(consts.MidasBuySerial, utils.GetCertificateSerialNumber(*MidasBuyCertificate))

	nonce := "this-is-a-nonce"
	w.Header().Set(consts.MidasBuyNonce, nonce)

	timestamp := strconv.FormatInt(time.Now().Unix(), 10)
	w.Header().Set(consts.MidasBuyTimestamp, timestamp)

	signature, _ := utils.SignSHA256WithRSA(
		fmt.Sprintf("%s\n%s\n%s\n", timestamp, nonce, body), MidasBuyPrivateKey)
	w.Header().Set(consts.MidasBuySignature, signature)
}
...
func assertAuthorization(t *testing.T, schema, method, uri string, params signParameter, body []byte) {
	assert.Equal(t, schema, "TXGW-SHA256-RSA2048")
	assert.Equal(t, params["auth_id"], testMchID)
...
func TestClientNoAuth(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(401)
		w.Header().Set(consts.RequestID, "0")
		fmt.Fprint(w, `{"name":"SIGN_ERROR","message":"sign error"}`)
	}))
	defer ts.Close()
	...
	assert.Equal(t, "SIGN_ERROR", apiError.Name)
...
func TestClient_Upload(t *testing.T) {
	...
	verifier = verifiers.NewSHA256WithRSAVerifier(
		core.NewCertificateMap(
			map[string]*x509.Certificate{utils.GetCertificateSerialNumber(*MidasBuyCertificate): MidasBuyCertificate},
		),
	)

	client, err := core.NewClient(ctx, option.WithSigner(signer))
	require.NoError(t, err)
